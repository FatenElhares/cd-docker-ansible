...
describe('Update Todo Item', function() {
  ...
  it('should have completed set true after PUT update', function() {
    var result = update(location, 'PUT', {'completed': true});
    return assert(result, "body.completed").to.be.true;
  });

  it('should have completed set true after PATCH update', function () {
...
function assert(result, prop){
  return exept(result).to.eventually.have.deep.property(prop)
}
